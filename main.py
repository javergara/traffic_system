...
def login(form: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    acces_token = {
        "sub": str(db_agent.agent_identifier),
...
def create_vehicle_for_person(
    person_id: int, vehicle: schemas.VehicleCreate, db: Session = Depends(get_db)
):
...
def create_agent(agent: schemas.AgentCreate, db: Session = Depends(get_db)):
    db_vehicle = crud.get_agent_by_identifier(
        db, agent_identifier=agent.agent_identifier
    )
    if db_vehicle:
